...
# -*- coding: utf-8 -*-
"""
35_train_boundary_only_center_loss.py

Test 2: boundary-only center-loss ablation for C2 D3.
...
It changes only a small center loss on the CLS embedding. A local control run with center_loss_lambda=0.0
...
Then it optionally launches:
  ...
  - attention/gradient/occlusion audit for the control and best center-loss run
"""
...
def forward_with_cls(model: nn.Module, tokens: torch.Tensor, z_values: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
    """Run the exact D3 forward path but also return CLS embedding.

    This is intentionally not an architecture change: the classifier and inference path
    remain identical. The CLS vector is used only for the auxiliary center loss during training.
    """
    cell_emb = model.embedding(tokens, z_values)
...
class CenterLoss(nn.Module):
    """Simple train-only class-center compactness loss on CLS embeddings.

    The center parameters are not used at inference. Loss is normalized by hidden_dim
    via mean over dimensions, so lambda values are interpretable relative to CE.
    """
    def __init__(self, num_classes: int, hidden_dim: int) -> None:
        super().__init__()
        self.centers = nn.Parameter(torch.zeros(int(num_classes), int(hidden_dim)))
        nn.init.normal_(self.centers, mean=0.0, std=0.02)

    def forward(self, cls_out: torch.Tensor, y: torch.Tensor) -> torch.Tensor:
        c = self.centers[y.long()]
        return ((cls_out - c) ** 2).mean(dim=1).mean()
...
def evaluate(model: nn.Module, loader: DataLoader, device: torch.device, labels: List[str], class_weights: torch.Tensor, label_smoothing: float) -> Dict[str, Any]:
    ...
    with torch.no_grad():
        for xb, vv, yb in loader:
            ...
            logits = model(xb, vv)
            loss = F.cross_entropy(logits, yb, weight=weight, label_smoothing=0.0)
...
def train_one_run(
    *, root: Path, train_mod, data: Dict[str, np.ndarray], meta: Dict[str, Any], labels: List[str],
    run_name: str, center_loss_lambda: float, args: argparse.Namespace, device: torch.device,
) -> Dict[str, Any]:
    ...
    center_loss_module = CenterLoss(len(labels), cfg["hidden_dim"]).to(device)
    if float(center_loss_lambda) > 0.0:
        optimizer = torch.optim.AdamW([
            {"params": model.parameters(), "weight_decay": float(args.weight_decay), "lr": float(args.lr)},
            {"params": center_loss_module.parameters(), "weight_decay": 0.0, "lr": float(args.lr)},
        ])
    else:
    ...
    write_json(run_dir / "config.json", {
        "run_name": run_name,
        "test": "boundary_only_center_loss",
        "center_loss_lambda": float(center_loss_lambda),
        "label_smoothing": 0.0,
        "seed": int(args.seed),
        "lr": float(args.lr),
        ...
        "fixed_components": ["C2 tokenization", "D3 architecture", "class weights", "dropout/model size", "optimizer schedule", "label_smoothing=0"],
        "changed_component": "center_loss_on_CLS_only",
    })
    ...
    for epoch in range(1, int(args.epochs) + 1):
        ...
        for xb, vv, yb in train_loader:
            ...
            if float(center_loss_lambda) > 0.0:
                logits, cls_out = forward_with_cls(model, xb, vv)
                ce_loss = F.cross_entropy(logits, yb, weight=weight_dev, label_smoothing=0.0)
                c_loss = center_loss_module(cls_out, yb)
                loss = ce_loss + float(center_loss_lambda) * c_loss
            else:
                logits = model(xb, vv)
                ce_loss = F.cross_entropy(logits, yb, weight=weight_dev, label_smoothing=0.0)
            ...
            seen += int(yb.numel())
        train_metrics = evaluate(model, train_eval_loader, device, labels, class_w, 0.0)
        val_metrics = evaluate(model, val_loader, device, labels, class_w, 0.0)
        row = {
            "epoch": epoch,
        ...
        if val_metrics["macro_f1"] > best_val:
            ...
            torch.save({
                ...
                "center_loss_lambda": float(center_loss_lambda),
                "label_smoothing": 0.0,
                "center_state_dict": {k: v.detach().cpu() for k, v in center_loss_module.state_dict().items()},
...
def main() -> None:
    ...
    ap.add_argument("--out-root", default="03_outputs/train_runs_test2_boundary_only")
    ap.add_argument("--audit-root", default="03_outputs/audit_test2_boundary_only")
    ...
    print(f"[test2] root={root}", flush=True)
    print(f"[test2] device={device}", flush=True)
    ...
    print(f"[test2] labels={labels}", flush=True)

    specs = [
        ("T2CTRL_C2_D3_CENTER000", 0.00, "local control: same runner, no center loss"),
        ("T2A_C2_D3_CENTER001", 0.01, "boundary-only: center loss lambda 0.01"),
        ("T2B_C2_D3_CENTER003", 0.03, "boundary-only: center loss lambda 0.03"),
    ]
    rows = []
    for run_name, cl, note in specs:
        print(f"\n========== RUN {run_name} center_loss_lambda={cl} ==========" , flush=True)
        row = train_one_run(root=root, train_mod=train_mod, data=data, meta=meta, labels=labels, run_name=run_name, center_loss_lambda=cl, args=args, device=device)
        row["note"] = note
        rows.append(row)

    summary_dir = ensure_dir(root / args.audit_root / "00_test_result_summary")
    pd.DataFrame(rows).to_csv(summary_dir / "test2_result_summary.csv", index=False)
    write_json(summary_dir / "test2_result_summary.json", rows)

    # Compare each LS run to local control and old C2 if available.
    comp = []
    ctrl = next(r for r in rows if r["center_loss_lambda"] == 0.0)
    ...
    for r in rows:
        c = {"run_name": r["run_name"], "center_loss_lambda": r["center_loss_lambda"]}
        ...
        comp.append(c)
    pd.DataFrame(comp).to_csv(summary_dir / "test2_delta_vs_control.csv", index=False)
    ...
    if not args.skip_attn_grad_audit:
        if not attn_script.exists():
            ...
            # Run logit/occlusion audit for local control and best non-control center-loss run.
            center_runs = [r for r in rows if r["center_loss_lambda"] > 0]
            best_center = sorted(center_runs, key=lambda x: x["val_macro_f1"], reverse=True)[0] if center_runs else None
            selected = [ctrl] + ([best_center] if best_center is not None else [])
            seen = set()
            for r in selected:
    ...
    lines = [
        ...
        "",
        "Fixed: C2 tokenization, D3 architecture, class weights, optimizer/dropout/model size, label_smoothing=0.",
        "Changed only: center loss on CLS embedding during training.",
        "",
        "## Result summary",
        "",
    ]
    ...
    lines += ["", "## Interpretation checklist", "", "- Did CLS centroid shift / amplification decrease?", "- Did val macro-F1 or malware-only F1 improve?", "- Did CLS amplification/mixed→pred decrease in overfit audit?", "- Did keep_current wrong-margin decrease without destroying malware F1?", "- Did any pair simply trade errors to another malware class?"]
    (summary_dir / "test2_readme_for_analysis.md").write_text("\n".join(lines), encoding="utf-8")

    zip_path = Path("/kaggle/working/test2_boundary_only_outputs.zip") if Path("/kaggle/working").exists() else root / "test2_boundary_only_outputs.zip"
