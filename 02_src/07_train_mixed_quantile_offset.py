...
# -*- coding: utf-8 -*-
"""
Train Transformer on mixed quantile/uniform bin + offset representation.

Does not modify baseline train/model files.
Reuses metric/logging helpers from 05_train.py.
"""
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Train mixed quantile-offset Transformer.")
    ...
    p.add_argument("--out-root", default=str(cfg("OUTPUT_ROOT", Path("03_outputs")) / "train_runs_mixed_quantile_offset"))
...
class BinOffsetDataset(Dataset):
    def __init__(self, X_bin: np.ndarray, X_offset: np.ndarray, y: np.ndarray) -> None:
        if X_bin.ndim != 2:
            raise ValueError(f"X_bin must be [N,F], got {X_bin.shape}")
        if X_offset.ndim != 2:
            raise ValueError(f"X_offset must be [N,F], got {X_offset.shape}")
        if X_bin.shape != X_offset.shape:
            raise ValueError(f"X_bin/X_offset mismatch: {X_bin.shape} vs {X_offset.shape}")
        ...
        self.O = torch.as_tensor(X_offset, dtype=torch.float32)
    ...
    def __getitem__(self, idx: int):
        return self.X[idx], self.O[idx], self.y[idx]


class ValueFeatureBinOffsetEmbedding(nn.Module):
    """
    value_emb = [offset] || Emb(bin_id)
    ...
    def __init__(
        ...
        *,
        ...
        if value_dim < 2:
            raise ValueError("value_dim must be >=2 because first coordinate is offset and rest is bin embedding.")
        if feature_dim < 1:
        ...
        self.bin_emb_dim = int(value_dim - 1)
    ...
    def forward(
        ...
        if offsets.ndim != 2:
            raise ValueError(f"offsets must be [B,F], got {tuple(offsets.shape)}")
        if tuple(bin_ids.shape) != tuple(offsets.shape):
            raise ValueError(f"bin_ids/offsets shape mismatch: {tuple(bin_ids.shape)} vs {tuple(offsets.shape)}")
        ...
        o = offsets.to(device=b.device, dtype=torch.float32).clamp(0.0, 1.0).unsqueeze(-1)

        bin_emb = self.bin_embedding(b)
        value_emb = torch.cat([o, bin_emb], dim=-1)
...
class BinOffsetTransformerClassifier(nn.Module):
    def __init__(
        self,
        *,
        num_bins: int,
        ...
        self.embedding = ValueFeatureBinOffsetEmbedding(
    ...
    def forward(self, tokens: torch.Tensor, z_values: Optional[torch.Tensor] = None, *, return_info: bool = False):
        # For compatibility with helpers from 05_train.py:
        # tokens   = bin_ids
        # z_values = offsets
        if z_values is None:
            raise ValueError("offsets must be passed through z_values argument.")
...
def make_diagnosis_summary(
    *,
    ...
    label_names = [label for label, _ in sorted(metadata["label_mapping"].items(), key=lambda kv: kv[1])]
    malware_labels = [x for x in label_names if x.lower() != "benign"]

    def malware_avg(rep: Dict[str, object]) -> float:
        vals = []
        for label in malware_labels:
            if label in rep["per_class"]:
                vals.append(float(rep["per_class"][label]["f1"]))
        return float(np.mean(vals)) if vals else 0.0

    return {
        "best_epoch": int(best_epoch),
        "representation": "mixed_quantile_offset",
        "train": {
            "loss": float(best_train["loss"]),
            "accuracy": float(best_train["accuracy"]),
            "macro_f1": train_macro,
            "weighted_f1": float(best_train["weighted_f1"]),
            "malware_only_avg_f1": malware_avg(best_train),
        },
        "val": {
            ...
            "malware_only_avg_f1": malware_avg(best_val),
        },
        ...
        "baseline_to_compare": {
            "name": "current z_continuous + uniform_128bin baseline",
...
def main() -> None:
    ...
    if not run_name:
        ...
        run_name = f"{timestamp}_mixed_quantile_offset_K{K}_B{B}_seed{args.seed}"
    ...
    config_obj = {
        "stage": "train_mixed_quantile_offset",
        "run_name": run_name,
        "K": K,
        "num_bins": B,
    ...
    train_ds = BinOffsetDataset(X_train, O_train, y_train)
    val_ds = BinOffsetDataset(X_val, O_val, y_val)
    ...
    model = BinOffsetTransformerClassifier(
        num_bins=B,
        n_features=n_features,
        num_classes=num_classes,
        value_dim=int(args.value_dim),
    ...
    print("===== mixed quantile offset training start =====")
    print(f"run_dir: {out_dir}")
    print(f"device: {device}")
    print(f"dataset: {dataset_path}")
    print(f"train bin/offset shape: {X_train.shape}/{O_train.shape}")
    print(f"val bin/offset shape:   {X_val.shape}/{O_val.shape}")
    ...
    print("===== mixed quantile offset training done =====")
